...
supabase.auth.onAuthStateChange((event, session) => {
  if (event === 'SIGNED_OUT') {
    // Sesión cerrada: limpia localStorage y redirige a index
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  btnLogout.addEventListener('click', async () => {
    ...
    // onAuthStateChange se encargará de limpiar y redirigir
  });
  ...
  async function loadInvoices() {
    ...
    facturas.forEach(f => {
      const tr = document.createElement('tr');
      tr.innerHTML = `
        <td><a href="#" onclick='openInvoice("${f.id}")'>${f.id}</a></td>
        <td>${f.user_id}</td>
  ...
  // — Eliminar producto (detalle + producto) —
  window.deleteProduct = async id => {
    ...
    let { error } = await supabase
      ...
      .eq('producto_id', id);
    if (error) {
      return alert('No se pudo eliminar los detalles asociados.');
    }
